 ...
 useEffect(() => {
  ...
  return (
     <div>
        <Grid container>
            <Grid item xs={12} >
                <Typography variant="h5" className="header-message">Customer Support</Typography>
            </Grid>
        </Grid>
                ...
                <List>
                    {Object.keys(selectedUser).length ?
                     <ListItem button key={selectedUser.id}>
                        <ListItemIcon >
                            <Avatar alt={`${selectedUser.user_subscriber.first_name}`} src="https://www.flaticon.com/free-icon/user_149071?term=user+avatar&page=1&position=2&origin=tag&related_id=149071" />
                        </ListItemIcon>
                        <Grid container  sx={{marginLeft: ".75rem"}}>
                          <Grid item xs={12} >
                              ...
                              <Tag sx={{borderRadius: '10px'}}color="green">Subscriber</Tag>
                          </Grid>
                          <Grid item xs={12}>
                          </Grid>
                        </Grid>
                ...
                <List>
                  {convos.map((convo) => (
                    <ListItem button key={convo.id} onClick={()=> viewMessage(convo.user_id, convo)}>
                      <ListItemIcon >
                          <Avatar alt={`${convo.user_subscriber.first_name}`} src="https://www.flaticon.com/free-icon/user_149071?term=user+avatar&page=1&position=2&origin=tag&related_id=149071" />
                      </ListItemIcon>
                      <Grid container  sx={{marginLeft: ".75rem"}}>
                        <Grid item xs={12} >
                          ...
                          <ListItem key={message.id}>
                            <Grid container> 
                              <Grid className={message.user_id != user ? classes.container : classes.containerRight} item xs={12}>
                                  <Typography className={message.user_id != user ? classes.position : classes.positionRight}>{message.message}</Typography>
                              </Grid>
                              <Grid item xs={12}>
